...
turbo-version bump --minor

# Prerelease for testing
turbo-version bump --preid alpha

# Monorepo selective update
turbo-version bump --package=@ui/button --minor
```
...
```bash
turbo-version changelog [options]
```
**Options**:
| Flag | Description | Example |
|------|-------------|---------|
...
turbo-version changelog

# Custom range to file
turbo-version changelog --from v2.1.0 --to v2.3.0 --file CHANGES.md
```
...
```bash
turbo-version bump --minor && git push --follow-tags
```

### 2. Monorepo Hotfix
```bash
turbo-version bump --patch --package @lib/utils \
